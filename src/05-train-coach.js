...
export function findPassenger(passengers, name) {
  ...
  return passengers.find(
    (passenger) =>
      passenger?.name && passenger.name.toLowerCase() === name.toLowerCase(),
  );
}
...
export function getPassengerIndex(passengers, name) {
  ...
  return passengers.findIndex(passenger => passenger?.name && passenger.name.toLowerCase() === name.toLowerCase());
}
...
export function isAnyWaitlisted(passengers) {
  ...
  return passengers.some(passenger => passenger?.status && passenger.status.toLowerCase() === "waitlisted");
}
...
export function areAllConfirmed(passengers) {
  ...
  return passengers.every(passenger => passenger?.status && passenger.status.toLowerCase() === "confirmed");
}
...
export function getWaitlistedPassengers(passengers) {
  ...
  return passengers.filter(passenger => passenger?.status && passenger.status.toLowerCase() === "waitlisted");
}
